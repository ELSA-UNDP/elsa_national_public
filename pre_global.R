...
terra::terraOptions(tempdir = here::here(""),
                    steps = 4,
                    todisk = TRUE)
...
if (restorelock) {
  Rest <-
    terra::rast(here::here("data/elsa_inputs/", lockin_df$file_name[2]))
}
# Zones ####
prot_zone <-
...
zone_protect_PA <-
  ifel(isTRUE(not.na(zone_protect) | not.na(PA)), 1, NA)
if (restorelock) {
...
zone_restore_PA[PA == 1] <- NA
if (restorelock) {
  zone_restore_Rest <-
    ifel(isTRUE(not.na(zone_restore) | not.na(Rest)), 1, NA)
}
if (restorelock & palock) {
